...
addMapping('Helvetica-Bold', 0, 0, 'Helvetica-Bold')
...
def chrome(c, section, pgnum, data, accent=GOLD):
    ...
    tc(c, data.get('instagram','@coach.teka1'), W/2-60, FTR_H/2-4, 'Helvetica', 7.5, GOLD)
    tc(c, data.get('phone','01033047057'), W/2+60, FTR_H/2-4, 'Helvetica', 7.5, SILVER)
...
def p1_cover(c, data):
    ...
    ty=H-120
    tc(c,data.get('program','PUSH // PULL // LEGS').replace('//','●'),W/2,ty,'Helvetica-Bold',36,WHITE)
    rrect(c,W/2-35,ty-35,70,18,3,GOLD_DIM,GOLD,0.8)
    tc(c,'VOL.1',W/2,ty-29,'Helvetica-Bold',9,GOLD)
    by=130
...
def p2_intro(c, data):
    ...
    x,y,cw=content_area()
    tl(c,'PROGRAM',x,y,'Helvetica-Bold',26,SILVER)
    tw=c.stringWidth('PROGRAM ','Helvetica-Bold',26)
    tl(c,'OVERVIEW',x+tw,y,'Helvetica-Bold',26,WHITE)
    hline(c,x,y-7,200,GOLD,1.5)
    wrap(c,data.get('philosophy',''),x+10,y-30,cw-20,'Helvetica',8.8,SILVER,lh=15)
    stats_y=FTR_H+M+52
...
def p_exercise(c, data, day_key, pgnum, accent=GOLD):
    ...
    tc(c,'EXERCISE SESSION',x+cw/2,y-32,'Helvetica',10,accent)
    ...
    for idx,ex in enumerate(day_exercises[:8]):
        ...
        tl(c,desc,cx_arr[0]+10,ey-24,'Helvetica',6.5,GRAY)
        ...
        tc(c,'●',ccx[4],ey-17,'Helvetica-Bold',12,accent)
        ey-=ROW_H
...
def p7_tips(c, data):
    ...
    for i,tip in enumerate(tips):
        ...
        tc(c,str(i+1),tx+20,ty-22,'Helvetica-Bold',9.5,GOLD)
...
def p8_coach(c, data):
    ...
    tc(c,data.get('instagram','@coach.teka1'),W/2-60,BOT_BAND/2-4,'Helvetica-Bold',11,WHITE)
    tc(c,data.get('phone','01033047057'),W/2+60,BOT_BAND/2-4,'Helvetica-Bold',11,WHITE)
