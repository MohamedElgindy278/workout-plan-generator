...
with st.form('nutrition_form'):
  ...
  st.markdown('---')
st.markdown('## 🍽️ MEALS')
st.markdown('<p class="section-desc">Configure each meal with custom icon and dynamic ingredients.</p>', unsafe_allow_html=True)

num_meals = st.number_input('Number of meals', 1, 8, 4, help='How many meals in the plan? (1-8)')
meals = []

# Initialize ingredients count for each meal
for i in range(8):
    if f'ingredients_count_{i}' not in st.session_state:
        st.session_state[f'ingredients_count_{i}'] = 4
...
for i in range(int(num_meals)):
    ...
    with mc3:
        meal_icon = st.text_input(f'🖼️ Icon {i+1}',
            value=['🥣', '💪', '🍗', '🥗', '🍝', '🥤', '🍎', '🥜'][i] if i < 8 else '🍽️',
            key=f'micon_{i}', help='Emoji icon for this meal. You can copy any emoji from emojipedia.org')
    
    # Row 2: Calories + Protein + Carbs + Fat
    ...
    with mc4:
        meal_protein = st.text_input(f'🥩 Protein (g) {i+1}', value=['36','55','45','42'][i] if i < 4 else '30', key=f'prot_{i}', help=f'Protein in grams.')
    with mc5:
        meal_carbs = st.text_input(f'🍚 Carbs (g) {i+1}', value=['110','110','55','15'][i] if i < 4 else '40', key=f'carb_{i}', help=f'Carbs in grams.')
    with mc6:
        meal_fat = st.text_input(f'🧈 Fat (g) {i+1}', value=['18','36','18','12'][i] if i < 4 else '15', key=f'fat_{i}', help=f'Fat in grams.')
    ...
    # Row 4: Dynamic Ingredients
    st.markdown(f'**🛒 Ingredients for Meal {i+1}**')
    
    ing_col1, ing_col2, ing_col3 = st.columns([1, 1, 4])
    with ing_col1:
        if st.form_submit_button(f'➕ Add Ingredient', use_container_width=True, key=f'add_ing_{i}'):
            st.session_state[f'ingredients_count_{i}'] += 1
    with ing_col2:
        if st.form_submit_button(f'➖ Remove', use_container_width=True, key=f'rem_ing_{i}') and st.session_state[f'ingredients_count_{i}'] > 1:
            st.session_state[f'ingredients_count_{i}'] -= 1
    ...
    for j in range(st.session_state[f'ingredients_count_{i}']):
        default_val = default_ingredients[i][j] if i < 4 and j < len(default_ingredients[i]) else f'مكون {j+1}'
        ing = st.text_input(f'Ingredient {j+1}', value=default_val, key=f'ing_{i}_{j}', 
                           help=f'Ingredient {j+1} for meal {i+1}.', label_visibility='collapsed')
        if ing.strip():
