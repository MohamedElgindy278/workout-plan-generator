...
with st.form('nutrition_form'):
    ...
    # Number of meals - using session state
    st.number_input('Number of meals', 1, 8, st.session_state.num_meals, key='num_meals_input',
                    help='How many meals in the plan? (1-8)', on_change=lambda: st.session_state.update(
                        {'num_meals': st.session_state.num_meals_input}))
    
    meals = []
    food_icons = ['🥣', '💪', '🍗', '🥗', '🍝', '🥤', '🍎', '🥜']
    
    for i in range(st.session_state.num_meals):
        ...
        with mc3:
            meal_icon = st.text_input(f'🖼️ Icon {i+1}',
                ...
                key=f'micon_{i}', help='Emoji icon for this meal.')
        ...
        for j in range(st.session_state[f'ing_count_{i}']):
            ...
            ing = st.text_input(f'Ingredient {j+1}', value=default_val, key=f'ing_{i}_{j}', label_visibility='collapsed')
            if ing.strip():
    ...
    with sup_col1:
        if st.form_submit_button('➕ Add Supplement', use_container_width=True):
    ...
    with pw_col1:
        if st.form_submit_button('➕ Add Pre-Workout', use_container_width=True):
    ...
    with rec_col1:
        if st.form_submit_button('➕ Add Recipe', use_container_width=True):
    ...
    for i in range(st.session_state.recipes_count):
        ...
        with rc4:
            ricon = st.text_input(f'Icon {i+1}', value=['🍳', '🥩', '🍚', '🥗', '🥤', '🍎'][i] if i < 6 else '🍽️', key=f'ricon_{i}')
        if rname.strip():
            recipes.append({'name': rname.strip(), 'desc': rdesc.strip(), 'link': rlink.strip(), 'icon': ricon.strip()})
